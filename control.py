...
class Control:
    ...
    def __init__(self, data, ui):
        ...
        #self.ui.p.child('select folder').sigActivated.connect(self.auto)
    ...
    def auto_cut_pitch(self, x,  y, frac, it, low_cut_coeff, high_cut_coeff):
        ...
        # -- filtern --
        y_lowess = self.lowess_filter(y, frac, it)
    ...
    def auto(self):
        ...
        for key in self.data.files:
            x, y = self.data.loadfile(os.path.join(directory, key))
            ...
            #self.ui.rio = pg.ROI([autox[0], autoy[0]], [autox[-1] - autox[0], 0], resizable=False, removable=True)
            #self.ui.rio.addRotateHandle([1, 0], [0, 1])
            #self.ui.rio.addTranslateHandle([0, 0], [0.5, 0.5])
            #self.data.rioarray.append(self.ui.rio)
    ...
    def onSelectBtn(self):
        ...
        for key in self.files:
            ...
            self.ui.rio = pg.ROI([x[0], y[0]], [x[-1] - x[0], 0], resizable=False, removable=True)
            self.ui.rio.addRotateHandle([1, 0], [0, 1])
            self.ui.rio.addTranslateHandle([0, 0], [0.5, 0.5])
            self.data.rioarray.append(self.ui.rio)
            print('self.data.rioarray = ', len(self.data.rioarray))
        ...
        print('self.data.xscalearray = ', self.data.xscalearray)
        #print('self.data.yarray = ',self.data.yarray)
        self.onFilesReady(directory)
